...
function getPriority(grade, points, dueDate) {
  ...
  if (grade >= 90) pointUrgency += 1;
  else if (grade >= 80) pointUrgency += 2;
  else if (grade >= 70) pointUrgency += 5;
  else pointUrgency += 8;

  if (points <= 10) pointUrgency += 1;
  else if (points <= 30) pointUrgency += 2;
  else if (points <= 50) pointUrgency += 3;
  else if (points <= 100) pointUrgency += 4;
  else pointUrgency += 5;

  // due date variable: value of select box
  const now = new Date();
  if (dueDate >= now.setDate(now.getDate() + 14)) pointUrgency += 1;
  else if (
    dueDate >= now.setDate(now.getDate() + 7) &&
    dueDate < now.setDate(now.getDate() + 14)
  )
    pointUrgency += 2;
  else if (
    dueDate > now.setDate(now.setDate() + 3) &&
    dueDate < now.setDate(now.getDate() + 7)
  )
    pointUrgency += 3;
  else if (
    dueDate >= now.setDate(now.setDate() + 2) ||
    dueDate < now.setDate(now.setDate() + 3)
  )
    pointUrgency += 4;
  else if (dueDate == now.setDate(now.setDate() + 1)) pointUrgency += 5;
  else if (dueDate == now.setDate(now.setDate() + 0)) pointUrgency += 8;
